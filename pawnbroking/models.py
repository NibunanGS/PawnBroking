...
class City(models.Model):
    ...
    post = models.ForeignKey('self', null=True)
...
class Customer(models.Model):
    ...
    def __hash__(self):
        return hash(self.name_father_town)

    def __eq__(self, other):
        return self.name_father_town == other.name_father_town
...
class Pledge(models.Model):
    ...
    loan_date = models.DateField(default=datetime.datetime.now(), db_index=True)
    ...
    def clean(self):
        if self.id is None:
            if not self.pledge_no.isdigit():
                if not (self.pledge_no[0].isalpha() and self.pledge_no[1:].isdigit()):
                    raise ValidationError("Pledge_no can have alphabet only has the first character")
    ...
    def save(self, *args, **kwargs):
        ...
        if self.principle > 5000 and self.principle <= 10000:
            self.document = 50
        elif self.principle > 10000:
            self.document = (self.principle * Decimal('0.5')) / 100
        else:
            self.document = self.principle / 100
...
class Redemption(models.Model):
    date = models.DateField(default=datetime.datetime.now(), verbose_name="Redemption Date", db_index=True)
...
class AuctionNotice(models.Model):
    from_date = models.DateField()
    to_date = models.DateField()
    notice_type = models.ForeignKey(NoticeTemplate)
    cost = models.PositiveIntegerField(default=20)
    grace_date = models.DateField()
    no_of_pledges = models.PositiveIntegerField()
    ...
    def clean(self):
        if self.from_date > self.to_date:
            raise ValidationError("From Date should be lesser than To Date")
        
        if self.grace_date <= datetime.date.today():
            raise ValidationError("Grace Date should be some future date")
        
    def save(self, *args, **kwargs):
        pledges = Pledge.objects.filter(status='Open', loan_date__range=(self.from_date, self.to_date))
        self.no_of_pledges = len(pledges);
        self.no_of_customers = self.__get_no_of_customers(pledges)
        
        super(AuctionNotice, self).save(* args, **kwargs)
    
    def __unicode__(self):
        return "Notice Date: " + str(self.created);
    
    def __get_no_of_customers(self, pledges):
        cust_vs_pledges = dict();
        for p in pledges:
            cust_vs_pledges.setdefault(p.customer, list()).append(p)
        return len(cust_vs_pledges)
    
class AuctionNoticeAndPledgeMap(models.Model):
    pledge_notice = models.CharField(max_length=100, unique=True) 
    notice = models.ForeignKey(NoticeTemplate)
    pledge = models.ForeignKey(Pledge)
    
    def save(self, *args, **kwargs):
        self.pledge_notice = str(self.pledge.id) + "-" + str(self.notice.id)
        super(AuctionNoticeAndPledgeMap, self).save(* args, **kwargs)
